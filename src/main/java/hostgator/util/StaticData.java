package hostgator.util.com.hostgator.util;

public class StaticData {

	//Pkg signup  
	public static final String sharedPkg     = "/signup/shared/29/36/SHARED3660/";
	public static final String dediPkg       = "/signup/dedicated/85/36/SNAPPY";
	public static final String wordPressPkg  = "/signup/wordpress/83021/36/SNAPPYW579";
	public static final String cloudSitesPkg = "/signup/cloudsites/90106/36/SNAPPY";	
	public static final String vpsPkg        = "/vps/82/36/SNAPPYV2/";
	public static final String resellerPkg        = "/signup/reseller/7/36/SNAPPYR192424";

	//Default Accounts
	public static final String resellerDefaultEmail = "resellerpackage@hostgator.com";
	public static final String sharedDefaultEmail   = "sharedpackage@hostgator.com";
	public static final String owpDefaultEmail      = "wordpresspackage@hostgator.com";
	public static final String defaultPassword      = "TEST_PASSWORD";
	public static final String defaultPin           = "4111";

	//Regflow
	public static final String domainName = "hgtest";
	public static final String userName   = "hg";
	public static final String pin        = "1234";
	public static final String firstName  = "HGQATeamTestAccount";
	public static final String lastName   = "HoustonOffice";
	public static final String phone      = "7134567890";
	public static final String address1   = "5005 Mitchelldale street";
	public static final String address2   = "200";
	public static final String city       = "Houston";
	public static final String zip        = "77092";
	
	//Test CC info
	public static final String testCreditCardNumber = "4111111111111111";
	public static final String testCreditCardName   = "Test Card";
	public static final String testCreditCardCVV    = "111";
	
	//Paypal Data
	public static final String payPalEmail    = "robert.stone+paypal.buyer@endurance.com";
	public static final String payPalPassword = "hksjaHdaWI$&9837";
	
	//DefaultAccounts
	public static final String sharedDefaultAccount    = "sharedpackage@hostgator.com";
	public static final String resellerDefaultAccount  = "resellerpackage@hostgator.com";

	public static final String portalPassword = "TEST_PASSWORD";
	
}
